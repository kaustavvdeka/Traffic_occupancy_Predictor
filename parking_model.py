...
# ============================================================================
# CLUSTERING & FORECASTING MODELS
# ============================================================================

class DemandClusterer:
    """Groups parking sites into density/congestion clusters using DBSCAN"""
    def __init__(self):
        # eps=0.015 mostly clusters sites within ~1.5km depending on lat/lng scaling
        self.model = DBSCAN(eps=0.015, min_samples=2)
        
    def cluster_sites(self, sites_data):
        if not sites_data or len(sites_data) < 2:
            return {s['id']: 0 for s in sites_data}
            
        # Feature 1: lat, Feature 2: lng, Feature 3: occupancy
        coords = np.array([[s.get('latitude', 0), s.get('longitude', 0), s.get('occupancy_percent', 0)/100.0] for s in sites_data])
        clusters = self.model.fit_predict(coords)
        return {s['id']: int(c) for s, c in zip(sites_data, clusters)}

class ForecastingModel:
    """Short-term forecasting model using RandomForest"""
    def __init__(self):
    ...
    def train(self, df):
        if len(df) > 10:
            features = df[['hour', 'day_of_week', 'current_occupancy']]
            target = df['future_occupancy']
            self.model.fit(features, target)
            self.is_trained = True
            
    def predict(self, current_occupancy_ratio, hour, day_of_week, minutes_ahead):
        if not self.is_trained:
            # Fallback heuristic if no historical data to train XGBoost
            fluctuation = np.random.uniform(-0.02, 0.02)
            # simulate predictable trend: peak demand between 9-11 and 17-19
            # If current time is approaching peak, occupancy goes up. 
            if (8 <= hour < 10) or (16 <= hour < 18):
                trend = 0.08
            elif (11 <= hour <= 14) or (20 <= hour <= 22):
                trend = -0.05
            else:
                trend = -0.01

            pred = current_occupancy_ratio + (trend * (minutes_ahead / 15.0)) + fluctuation
            return max(0.0, min(1.0, pred))
            
        df = pd.DataFrame([{
            'hour': hour,
            'day_of_week': day_of_week,
            'current_occupancy': current_occupancy_ratio
        }])
...
def main():
    ...
    # Paths
    train_folder = "parkingg/train"
    test_folder = "parkingg/test"
